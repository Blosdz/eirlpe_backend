...
let TenantConnectionService = TenantConnectionService_1 = class TenantConnectionService {
    ...
    async getConnection(hostname) {
        ...
        const dataSource = new typeorm_1.DataSource({
            ...
            database: dbName,
            entities: [tenant_entities_1.TenantContact, tenant_entities_1.TenantUser],
            synchronize: false,
    ...
    async tenantDatabaseExists(hostname) {
        ...
        try {
            await adminDataSource.initialize();
            const result = await adminDataSource.query(`SELECT 1 FROM pg_database WHERE datname = $1`, [dbName]);
            await adminDataSource.destroy();
